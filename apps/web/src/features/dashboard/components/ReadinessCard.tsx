...
export function ReadinessCard({ userId }: ReadinessCardProps = {}) {
  ...
  return (
    <div className="bg-gradient-to-br from-surface-dark via-surface-dark to-surface-dark/50 border border-primary/20 rounded-3xl p-8 backdrop-blur-xl shadow-2xl hover:shadow-primary/10 transition-all duration-500 hover:scale-[1.02] group relative overflow-hidden isolate">
      {/* Animated background gradient */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${status.bg} opacity-0 group-hover:opacity-100 rounded-3xl transition-opacity duration-700 -z-10 pointer-events-none`}
      ></div>
