  ...
  return (
    <div className="bg-gradient-to-br from-[#0a0a0f] via-[#0f0f14] to-[#14141a] text-slate-200 font-display antialiased h-screen overflow-hidden flex relative">
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 lg:hidden p-3 bg-surface-dark/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-lg"
      >
        <span className="material-icons text-white">{mobileSidebarOpen ? 'close' : 'menu'}</span>
      </button>
      ...
      <aside
        className={`
          ...
          w-72 flex-shrink-0 border-r border-white/10 bg-black/95 lg:bg-black/40 backdrop-blur-xl
      ...
      >
        <div>
          {/* Logo */}
          <div className="h-20 flex items-center justify-start px-6 border-b border-white/10 bg-gradient-to-r from-primary/10 to-transparent">
            ...
            <span className="ml-4 font-black text-2xl tracking-tight bg-gradient-to-r from-white to-primary bg-clip-text text-transparent drop-shadow-lg">
              TrailQuest
            </span>
          </div>
          ...
          <nav className="mt-8 px-4 space-y-2">
            {navItems.map(item => (
              ...
              >
                ...
                <span className="ml-4 font-semibold tracking-wide">{item.label}</span>
                {isActive(item.path) && (
                  <div className="ml-auto w-1.5 h-8 bg-primary rounded-full shadow-lg shadow-primary/50"></div>
                )}
              </Link>
            ))}
            ...
            {profile?.role === 'trainer' && (
              <button
                ...
                className="w-full flex items-center px-4 py-3.5 rounded-xl group transition-all duration-300 text-slate-400 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10 relative"
              >
                <div className="w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 bg-white/5 text-slate-400 group-hover:bg-white/10 group-hover:text-white">
                  <span className="material-icons text-xl">chat</span>
                </div>
                <span className="ml-4 font-semibold tracking-wide">Chat with Trainees</span>
                {unreadCount > 0 && (
                  <span className="absolute top-2 right-4 w-5 h-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg shadow-red-500/50 animate-pulse">
            ...
            {(profile?.role === 'trainee' || profile?.role === 'hiker') && (
              ...
              >
                ...
                <span className="ml-4 font-semibold tracking-wide">Chat with Trainer</span>
                {unreadCount > 0 && (
                  <span className="absolute top-2 right-4 w-5 h-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg shadow-red-500/50 animate-pulse">
                    {unreadCount > 9 ? '9+' : unreadCount}
                  </span>
      ...
      <main className="flex-1 h-full overflow-y-auto overflow-x-hidden relative">
        ...
        <div className="relative z-10 p-4 sm:p-6 lg:p-10 max-w-[1600px] mx-auto pt-20 lg:pt-6">
          {children}
        </div>
      </main>
